...
BASE_DATASETS = [
    'CIFAR100', 'CIFAR100-C', 'CIFAR10-C', 'CIFAR100-LT', 'CIFAR10',
    'STL10', 'CIFAR20', 'IN1K', 'IN50', 'IN100', 'IN200', 'CINIC10',
    'SVHN', 'TinyImageNet', 'SUN397', 'DTD', 'EuroSAT',
    'food101', 'caltech101', 'tiny-imagenet', 'tiny-imagenet-200', 'imagenet'
]


def parse_dataset_name(dataset):
    """Split dataset variants like CIFAR100-LT_if0p1 into base + suffix."""
    match = re.fullmatch(r'(?P<base>[A-Za-z0-9-]+?)(?:_(?P<tag>.+))?', dataset)
    if match is None:
        return dataset, None
...
def validate_dataset_name(dataset):
    base_dataset, _ = parse_dataset_name(dataset)
    if base_dataset not in BASE_DATASETS:
        raise argparse.ArgumentTypeError(
            f"Unsupported dataset '{dataset}'. Base dataset must be one of: {', '.join(BASE_DATASETS)}"
        )
    return dataset
...
def get_args_parser():
    ...
    parser.add_argument('--embedding_path', default=None, type=str,
                        help='Path to precomputed train embeddings.pt. Used with --precomputed.')
    parser.add_argument('--label_path', default=None, type=str,
                        help='Optional labels path for --embedding_path. Defaults to label.pt or labels.pt next to embeddings.')
    parser.add_argument('--dataset', default='CIFAR100', type=validate_dataset_name,
                        help='Dataset name. Variants like CIFAR100-LT_if0p1 are also supported.')
...
def process_args(args):
    ...
    if args.loss == "TEMI" and "beta" not in args.loss_args:
        args.loss_args["beta"] = 0.6
    args.dataset_base, args.dataset_tag = parse_dataset_name(args.dataset)
    if args.embedding_path:
        from pathlib import Path
        embedding_path = Path(args.embedding_path).expanduser().resolve()
        args.embedding_path = str(embedding_path)
        if args.label_path is None:
            label_candidates = [
                embedding_path.with_name('label.pt'),
                embedding_path.with_name('labels.pt'),
            ]
            for label_path in label_candidates:
                if label_path.is_file():
                    args.label_path = str(label_path)
                    break
        if not args.knn_path:
            args.knn_path = embedding_path.with_name('knn.pt')
    if not args.knn_path:
        import gen_embeds
