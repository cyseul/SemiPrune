...
build_jobs() {
  local pair ratio mis_ratio seed score_metric

  for score_metric in "${SCORE_METRICS[@]}"; do
    while read -r ratio mis_ratio; do
      [[ -z "${ratio:-}" ]] && continue

      if ! valid_mis_ratio "${ratio}" "${mis_ratio}"; then
        echo "[SKIP] invalid pair: metric=${score_metric}, coreset_ratio=${ratio}, mis_ratio=${mis_ratio}" >&2
        continue
      fi
    ...
    done < <(get_pairs_for_metric "${score_metric}")
  done
}
