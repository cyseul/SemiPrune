...
BASE_DIR="./data-model/food101/elfs-ps/full-train"

# NOTE: check whether this should be .pt or .pt.pt
SCORE_PATH="${BASE_DIR}/food101_score_multiT.pt"
...
run_job() {
  ...
  local task_name="elfs-ps-${score_metric}-r${ratio_tag}-mr${mis_tag}-md${mis_descending}-s${seed}"
  ...
  python train.py \
    ...
    --gpuid "${gpu}" \
    --ignore-td \
    --wandb-project "food101_score_abl_elfs"
