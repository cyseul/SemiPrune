...
class ImageNetSubset(data.Dataset):
    def __init__(self, subset_file, root='../data/imagenet/train', split='train', 
                    transform=None):
...
class TinyImageNet(datasets.ImageFolder):
    def __init__(self, root='', split='train', transform=None):
        # split은 일단 interface 맞추려고 받기만 하고,
        # 실제로는 root 아래 폴더 전체를 ImageFolder로 읽음
        super().__init__(root=root, transform=None)
        self.transform = transform
        self.split = split
        self.resize = tf.Resize((64, 64))
