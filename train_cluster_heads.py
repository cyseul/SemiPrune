...
def ensure_knn_from_embeddings(args):
    if not args.precomputed or not getattr(args, "embedding_path", None):
        return
    ...
    print(f"knn_path not found. Building KNN from {args.embedding_path}")
    embeddings = torch.load(args.embedding_path, map_location="cpu")
    k = args.knn
    if k < 0:
        k = embeddings.size(0) - 1
    _, neighbors = compute_neighbors(embeddings, k)
    knn_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(neighbors.cpu(), knn_path)
...
def train_dino(args, writer):
    ...
    ensure_knn_from_embeddings(args)
